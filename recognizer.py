    ...
    if model:
        try:
            if os.path.exists(model_file):
                # Load existing models and add the new one
            ...
            result_text.insert(tk.END, "Model saved successfully.\n")
        except Exception as e:
            result_text.insert(tk.END, f"Error saving model: {e}\n")
    else:
        messagebox.showerror("Save Error", "No trained model to save!")
...
def load_model():
    ...
    try:
        ...
        if file_path:
            ...
            result_text.insert(tk.END, f"Model loaded successfully from {file_path}\n")
        else:
            result_text.insert(tk.END, "No file selected.\n")
    except Exception as e:
        result_text.insert(tk.END, f"Error loading model: {e}\n")
...
def test_model():
    global model
    if not model:
        messagebox.showerror("Model Error", "Please load or train a model first!")
        return
...
tk.Button(button_frame, text="Save Model", command=save_model).pack(side=tk.LEFT, padx=10)
tk.Button(button_frame, text="Load Model", command=load_model).pack(side=tk.LEFT, padx=10)
